import type {
  Background, BgStyle, BgType, DecoCat, DecoLayer, DecoPrim, DecoSet, DecoShape,
  DeviceKind, DeviceLayer, FitMode, LightType, Material, PatternKind, PosPreset,
  Project, ScreenRect, ShadowPreset, TextBlock, Mood,
} from './types';
...
export function deviceGeometry(kind: DeviceKind, w: number, h: number, radiusMul = 1): ScreenRect {
  switch (kind) {
    case 'laptop': {
      const baseH = h * 0.062; const lidH = h - baseH;
    ...
    case 'monitor': {
      const standH = h * 0.15; const screenH = h - standH; const pad = screenH * 0.028;
...
export const CANVAS_PRESETS = [
  { label: 'Showcase', w: 1600, h: 1000 }, { label: 'MacBook', w: 1440, h: 900 },
  { label: 'Portfolio', w: 1200, h: 800 }, { label: 'LinkedIn', w: 1200, h: 628 },
  { label: 'Square', w: 1080, h: 1080 }, { label: 'IG Portrait', w: 1080, h: 1350 },
  { label: 'Story', w: 1080, h: 1920 }, { label: 'Full HD', w: 1920, h: 1080 },
];
...
/* ---------------- background factory ---------------- */
export const bg = (type: BgType, c1: string, c2: string, c3: string, angle: number, pattern: PatternKind, po: number, style: BgStyle = 'plain'): Background => ({
...
export const DECO_PRESETS: DecoPresetDef[] = [
  ...
  dp('blob3d', 'Soft blob', '3d', 'blob'), dp('disc3d', 'Disc', '3d', 'disc'), dp('ribbon', 'Ribbon', '3d', 'ribbon'),
...
export function getDecoShapes(set: DecoSet, seed: number, w: number, h: number, intensity: number, c1: string, c2: string): DecoShape[] {
  ...
  const edgePos = () => {
    const side = Math.floor(rnd() * 4); const m = 0.16;
  ...
  for (let i = 0; i < n; i++) {
    const p = edgePos(); const color = colors[i % colors.length];
...
export function paletteToBg(pal: Palette, seed: number): Background {
  return { type: pal.type, c1: pal.c1, c2: pal.c2, c3: pal.c3, angle: pal.angle, pattern: pal.pattern, patternOpacity: pal.po, style: pal.style, seed, light: { type: pal.light, intensity: 0.55 }, meshPoints: 4 };
}
...
export function makeDevice(kind: DeviceKind, cw: number, ch: number, assetId: string | null, index: number): DeviceLayer {
  ...
  return {
    ...
    x: (cw - w) / 2, y: (ch - w / meta.aspect) / 2, w,
    tilt: 0, color: meta.colors[0].hex, assetId, fit: 'cover',
    zoom: 1, panX: 0, panY: 0, shadow: 'soft', url: 'yourapp.com', visible: true,
...
export function makeDefaultProject(name: string, type: string, cw: number, ch: number): Project {
  ...
  const text: TextBlock = {
    ...
    position: 'bottom-left', scale: 1, color: '#f2f0ea', autoColor: true, backplate: 'none',
  };
  return {
    id: uid(), name, type, createdAt: Date.now(), updatedAt: Date.now(),
    canvas: { w: cw, h: ch }, assets: [],
    devices: [makeDevice('laptop', cw, ch, null, 0)],
    background: { ...preset.bg, seed: Math.floor(Math.random() * 1e9), kind: 'procedural' },
    text,
    ...
    thumbnail: null, exportCount: 0, decos: [], mood: 'auto', icons: [],
  };
}
...
export function migrate(p: any): Project {
  ...
  return {
    ...
    devices: (p.devices || []).map((d: any) => ({ brightness: 1, reflection: 0, radiusMul: 1, opacity: 1, material: 'matte' as Material, z: 0, ...d })),
    decos: p.decos || [], mood: p.mood || 'auto', icons: p.icons || [],
    accents: p.accents || { a1: '#ff6b3d', a2: '#45d6c8' },
    text: { ...base.text, ...(p.text || {}) }, logo: { ...base.logo, ...(p.logo || {}) },
  };
}
...
export function applyLayoutPositions(p: Project, layoutId: string): Project {
  ...
  const devices: DeviceLayer[] = layout.devices.map((d, i) => {
    ...
    return {
      ...
      x: cw * d.fx, y: ch * d.fy, w: cw * d.fw,
      assetId: assets[i]?.id ?? prev?.assetId ?? null,
      color: prev?.color ?? DEVICE_META[d.kind].colors[0].hex,
    };
  });
