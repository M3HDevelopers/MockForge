...
/*
  Essential icon library — 100+ icons across key categories.
  Each icon is a single SVG path (24x24 viewBox, stroke-based).
  Data-driven: add more by appending to this array.
*/
export const ICONS: IconDef[] = [
  ...
  { id: 'typography', name: 'Typography', category: 'design', tags: ['typography', 'font', 'text'], d: 'M4 7V4h16v3M9 20h6M12 4v16', style: 'outline' },
...
export function iconsForProjectType(type: string): IconDef[] {
  const t = type.toLowerCase();
  if (t.includes('ecom') || t.includes('shop')) return ICONS.filter(i => i.category === 'ecom');
  if (t.includes('ai') || t.includes('ml')) return ICONS.filter(i => i.category === 'ai');
  if (t.includes('mobile') || t.includes('app')) return ICONS.filter(i => i.category === 'mobile');
  if (t.includes('dashboard') || t.includes('analytics')) return ICONS.filter(i => i.category === 'business' || i.category === 'ui');
  if (t.includes('saas') || t.includes('web')) return ICONS.filter(i => i.category === 'web' || i.category === 'cloud');
