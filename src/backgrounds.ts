import type { Background, BgStyle, ImageBgState } from './types';
import { isDark, luminance, mulberry32, rgba, shade } from './templates';
import { imgOf, loadImageEl, resolveImageSrc } from './images';

/* =========================================================================
   Background renderer — THREE engines in one:
   1. PROCEDURAL (vector base + style artwork + pattern + lighting)
   2. IMAGE (raster background + transforms + filters + overlays + blend + mask)
   3. HYBRID (image base + procedural artwork layered on top)
   Shared by the live preview and the export renderer (preview ≡ export).
   ========================================================================= */
...
export async function renderBackground(
  ...
  if (kind === 'image' || kind === 'hybrid') {
    ...
    if (src) {
      try { await drawImageBg(ctx, img, src, w, h); } catch { /* keep procedural base as graceful fallback */ }
    }
    if (kind === 'hybrid') {
      ctx.save(); ctx.globalAlpha = 0.55;
      if (b.style && b.style !== 'plain') paintStyle(ctx, b.style, b, w, h, accents);
      ctx.restore();
      paintPattern(ctx, b, w, h);
    }
    drawOverlays(ctx, img, w, h);
...
async function drawImageBg(ctx: CanvasRenderingContext2D, img: ImageBgState, src: string, w: number, h: number) {
  const el = await loadImageEl(src);
  const iw = el.naturalWidth, ih = el.naturalHeight;

  ctx.save();
  /* mask clip */
  if (img.mask === 'rounded') { ctx.beginPath(); ctx.roundRect(w * 0.03, h * 0.04, w * 0.94, h * 0.92, Math.min(w, h) * 0.05); ctx.clip(); }
  else if (img.mask === 'circle') { ctx.beginPath(); ctx.arc(w / 2, h / 2, Math.min(w, h) * 0.46, 0, Math.PI * 2); ctx.clip(); }

  /* compute destination rect */
  let dw = w, dh = h, dx = 0, dy = 0;
  const s = img.scale;
  if (img.fit === 'cover') {
    const sc = Math.max(w / iw, h / ih) * s; dw = iw * sc; dh = ih * sc;
    dx = (w - dw) / 2 + img.x * w * 0.3; dy = (h - dh) / 2 + img.y * h * 0.3;
  } else if (img.fit === 'contain') {
    const sc = Math.min(w / iw, h / ih) * s; dw = iw * sc; dh = ih * sc;
    dx = (w - dw) / 2 + img.x * w * 0.3; dy = (h - dh) / 2 + img.y * h * 0.3;
  } else if (img.fit === 'center') {
    dw = iw * s; dh = ih * s;
    dx = (w - dw) / 2 + img.x * w * 0.3; dy = (h - dh) / 2 + img.y * h * 0.3;
  } else { /* fill / stretch */
    dw = w * s; dh = img.fit === 'fill' ? h * s : (h * s) * (ih / ih); dh = h * s;
    dx = (w - dw) / 2 + img.x * w * 0.3; dy = (h - dh) / 2 + img.y * h * 0.3;
  }
  ...
  const flt = [
    `brightness(${img.brightness})`, `contrast(${img.contrast})`, `saturate(${img.saturation})`,
    img.blur > 0 ? `blur(${img.blur}px)` : '', img.hue !== 0 ? `hue-rotate(${img.hue}deg)` : '', preset,
  ].filter(Boolean).join(' ');
  ...
  /* radial / gradient mask fade */
  if (img.mask === 'radial' || img.mask === 'gradient') {
    ctx.globalCompositeOperation = 'destination-out';
    const g = img.mask === 'radial'
      ? ctx.createRadialGradient(w / 2, h / 2, Math.min(w, h) * 0.3, w / 2, h / 2, Math.max(w, h) * 0.62)
      : ctx.createLinearGradient(0, 0, 0, h);
    if (img.mask === 'gradient') { g.addColorStop(0, 'rgba(0,0,0,0)'); g.addColorStop(0.5, 'rgba(0,0,0,0)'); g.addColorStop(1, 'rgba(0,0,0,1)'); }
  ...
  if (img.tint && img.tintOpacity > 0) {
    ...
    ctx.fillStyle = img.tint; ctx.fillRect(0, 0, w, h);
    ctx.globalAlpha = 1;
  }
  ctx.restore();
}
...
/* ---------------- overlays ---------------- */
function drawOverlays(ctx: CanvasRenderingContext2D, img: ImageBgState, w: number, h: number) {
  const o = img.overlay;
  if (o === 'none') return;
  ctx.save();
  const a = img.overlayOpacity;
  if (o === 'color') { ctx.globalAlpha = a; ctx.fillStyle = img.overlayColor; ctx.fillRect(0, 0, w, h); }
  else if (o === 'black') { ctx.globalAlpha = a; ctx.fillStyle = '#000'; ctx.fillRect(0, 0, w, h); }
  else if (o === 'white') { ctx.globalAlpha = a; ctx.fillStyle = '#fff'; ctx.fillRect(0, 0, w, h); }
  else if (o === 'gradient') {
    ...
    const g = ctx.createRadialGradient(w / 2, h * 0.3, 0, w / 2, h * 0.3, Math.max(w, h) * 0.6);
    ...
    for (let i = 0; i < count; i++) { ctx.globalAlpha = a * rnd() * 0.5; ctx.fillRect(rnd() * w, rnd() * h, 1, 1); }
  }
  ctx.restore();
}
...
function paintBase(ctx: CanvasRenderingContext2D, b: Background, w: number, h: number) {
  if (b.type === 'solid') { ctx.fillStyle = b.c1; ctx.fillRect(0, 0, w, h); }
    ...
    for (let i = 0; i < n; i++) blob(rnd() * w, rnd() * h, Math.max(w, h) * (0.3 + rnd() * 0.25), i % 2 === 0 ? b.c2 : b.c3);
  }
}

/* ---------------- procedural style artwork ---------------- */
function paintStyle(ctx: CanvasRenderingContext2D, style: BgStyle, b: Background, w: number, h: number, accents: { a1: string; a2: string }) {
  ...
  switch (style) {
    ...
    case 'abstract': {
      ...
      ctx.beginPath(); ctx.moveTo(-min * 0.1, h * 0.55);
    ...
    case 'tech': {
      ...
      ctx.strokeStyle = rgba(ink, dark ? 0.05 : 0.07); ctx.lineWidth = 1; ctx.beginPath();
      for (let i = 0; i < n; i++) for (let j = i + 1; j < n; j++) {
        const d = Math.hypot(pts[i][0] - pts[j][0], pts[i][1] - pts[j][1]);
        if (d < min * 0.34) { ctx.moveTo(pts[i][0], pts[i][1]); ctx.lineTo(pts[j][0], pts[j][1]); }
      }
      ctx.stroke();
      for (const [x, y] of pts) { ctx.fillStyle = rgba(ink, dark ? 0.1 : 0.12); ctx.beginPath(); ctx.arc(x, y, 2.4, 0, Math.PI * 2); ctx.fill(); }
